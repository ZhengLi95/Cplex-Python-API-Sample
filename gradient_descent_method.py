...
def gradientDescentMethod(func, initialX, displayDetail= False, decimal= 3, accuracy= 1e-6):
    ...
    while True:
        ...
        negativeGrad = - numerical_differentiate.Grad(func, initialX)

        if displayDetail == True:
            print("NEG-GRADIENT AT POINT " + str(initialX.round(decimal))
                  + " IS " + str(negativeGrad.round(decimal)))

        optAlpha = linear_search.GoldenSectionMethod(func, initialX, negativeGrad, accuracy)
        ...
        if check_convergence.isConvergent(initialX, newX, accuracy= 1e-6):
            if displayDetail == True:
